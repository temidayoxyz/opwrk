...
export async function handleSystemBridgeMessage(
  ...
  switch (type) {
    ...
    case 'api:openchamber:update-check': {
      // The VS Code extension has no OpWrk distribution channel yet. Do not
      // send update checks or installation identifiers to the inherited service.
      return {
        id,
        type,
        success: true,
        data: {
          available: false,
          currentVersion: String(ctx?.context?.extension?.packageJSON?.version || 'unknown'),
