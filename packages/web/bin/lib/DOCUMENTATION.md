...
- `commands-connect-url.js`
  ...
  - `--relay` includes a relay fallback in the pairing link. OpWrk has no hosted relay default: set `OPENCHAMBER_RELAY_URL` or save a self-hosted endpoint first. Missing configuration fails before the command starts a server or creates a pairing session; the inherited OpenChamber relay URL is rejected. The running host must have relay enabled to serve redemption over that transport.

- `commands-update.js`
  - The inherited `openchamber update` command fails with a release link while OpWrk has no published CLI package; it never stops running instances or invokes an upstream package installer.
