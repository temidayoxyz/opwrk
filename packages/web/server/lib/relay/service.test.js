...
describe('relay service passive hosting', () => {
  it('does not connect to the inherited OpenChamber relay from saved settings', async () => {
    const { service, hostLock } = makeService({
      settings: { privateRelay: { enabled: true, relayUrl: 'wss://relay.openchamber.dev/ws' } },
    });
    try {
      ...
      await expect(service.ensureEnabledForPairing()).rejects.toThrow(/self-hosted relay URL/);
