...
export const createRelayService = ({
  ...
  const readConfig = async () => {
    ...
    const relayUrl = override ?? normalizeRelayUrl(stored?.relayUrl);
    return {
      enabled: stored?.enabled === true && Boolean(relayUrl),
      relayUrl,
  ...
  const start = async (relayUrl, { claim = 'try' } = {}) => {
    ...
    if (!isValidRelayUrl(relayUrl)) {
      throw new Error('Configure a self-hosted relay URL before enabling remote relay access');
    }
  ...
  const reconcile = async () => {
    try {
      ...
      if (demand) {
        if (!config.relayUrl) {
          throw new Error('Relay pairing requires a configured self-hosted relay URL');
        }
  ...
  const registerRoutes = (app) => {
    ...
    app.post('/api/openchamber/relay/enable', express.json({ limit: '16kb' }), async (req, res) => {
      try {
        ...
        if (!relayUrl) {
          res.status(400).json({ error: 'Configure a self-hosted relay URL before enabling remote relay access' });
          return;
        }
