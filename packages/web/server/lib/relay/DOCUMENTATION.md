...
- `service.js` — thin entrypoint: relay config (enabled flag + relay URL), the management routes (`GET/POST /api/openchamber/relay/{status,enable,disable}`), a `getPairingCandidate()` accessor (the relay transport candidate folded into pairing-v2 links when enabled, consumed by the pairing-session route in `core-routes.js`), and lifecycle wiring. OpWrk has no hosted relay default. A valid self-hosted `ws://` or `wss://` URL must be saved or supplied through `OPENCHAMBER_RELAY_URL` before relay pairing or hosting can start. The inherited `relay.openchamber.dev` endpoint is rejected, including in saved settings. The environment override takes precedence for the host connection, pairing candidate, and status.
