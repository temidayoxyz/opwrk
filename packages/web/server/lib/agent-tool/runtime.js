...
const createToolEntry = ({ name, description, actions, definitions, parameters }) => String.raw`    ${name}: {
      ...
      args: {
        action: { type: "string", enum: ${JSON.stringify(actions)}, oneOf: ${JSON.stringify(definitions.map((entry) => ({ const: entry.action, description: entry.description })))}, description: "OpWrk action to perform" },
      ...
      async execute(input, context) {
        ...
        if (!endpoint || !token) {
          return failure({ schemaVersion: ${TOOL_SCHEMA_VERSION}, ok: false, action: args.action, error: { message: "OpWrk managed tool connection is unavailable" } })
        }
        ...
        try {
          ...
          return failure({ schemaVersion: ${TOOL_SCHEMA_VERSION}, ok: false, action: args.action, error: { message: "OpWrk returned an invalid response", kind: "runtime", status: response.status } })
        } catch (error) {
...
export const createAgentToolRuntime = (dependencies) => {
  ...
  const prepareManagedOpenCodeEnv = async ({ includeControl = true, includeWeb = true, includeMemory = true } = {}) => {
    ...
    if (!Number.isInteger(port) || port <= 0) {
      throw new Error('OpWrk listener port is unavailable for managed tool injection');
    }
    if (!includeControl && !includeWeb && !includeMemory) {
      throw new Error('At least one OpWrk managed tool must be enabled to inject the plugin');
    }
  ...
  const execute = async (payload = {}, options = {}) => {
    ...
    if (!ACTIONS.has(action)) {
      return createResult({ ok: false, action, error: { message: `Unsupported OpWrk action: ${action}`, kind: 'usage' } });
    }
    if (typeof executeAction !== 'function') {
      return createResult({ ok: false, action, error: { message: 'OpWrk control service is unavailable', kind: 'runtime' } });
    }
    try {
