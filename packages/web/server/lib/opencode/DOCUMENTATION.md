...
- `packages/web/server/lib/opencode/openchamber-routes.js`: OpWrk update and models metadata route registration. Desktop-host updates use the native Electron updater. Non-desktop checks read OpWrk GitHub releases without an install identifier; web package-manager installation is disabled until an OpWrk package is published.
...
- `registerPwaManifestRoute(app, dependencies)`: registers PWA manifest endpoint with dynamic app-name resolution and recent-session shortcuts:
  ...
  - The default install name is OpWrk; an explicit user-saved PWA name still wins.
