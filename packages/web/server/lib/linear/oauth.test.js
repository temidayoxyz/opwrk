...
describe('Linear OAuth PKCE', () => {
  ...
  beforeEach(() => {
    ...
    previousBroker = process.env.OPENCHAMBER_LINEAR_BROKER_URL;
    dataDir = makeTempDir();
    process.env.OPENCHAMBER_DATA_DIR = dataDir;
    process.env.OPENCHAMBER_PORT = '3001';
    process.env.OPENCHAMBER_LINEAR_CLIENT_ID = 'opwrk-linear-test-client';
    delete process.env.OPENCHAMBER_LINEAR_BROKER_URL;
  ...
  afterEach(() => {
    ...
    restoreEnv('OPENCHAMBER_LINEAR_BROKER_URL', previousBroker);
    fs.rmSync(dataDir, { recursive: true, force: true });
  });

  it('does not contact a broker without an explicitly configured client id', async () => {
    delete process.env.OPENCHAMBER_LINEAR_CLIENT_ID;
    const network = vi.fn();
    vi.stubGlobal('fetch', network);
    await expect(startAuthorization()).rejects.toMatchObject({ code: 'LINEAR_CLIENT_ID_MISSING' });
    expect(network).not.toHaveBeenCalled();
  });

  it('does not contact a broker without an explicitly configured callback', async () => {
    delete process.env.OPENCHAMBER_LINEAR_REDIRECT_URI;
    const network = vi.fn();
    vi.stubGlobal('fetch', network);
    await expect(startAuthorization()).rejects.toMatchObject({ code: 'LINEAR_REDIRECT_URI_MISSING' });
    expect(network).not.toHaveBeenCalled();
  });
  ...
  it('creates an S256 authorize URL and stores a pending verifier', async () => {
    ...
    expect(url.searchParams.get('client_id')).toBe('opwrk-linear-test-client');
  ...
  it('claims a broker callback and exchanges it locally with PKCE', async () => {
    delete process.env.OPENCHAMBER_LINEAR_REDIRECT_URI;
    process.env.OPENCHAMBER_LINEAR_BROKER_URL = 'https://broker.example.test/v1/oauth/linear';
    const brokerAndTokenFetch = vi.fn(async (url, init) => {
      ...
      if (target.endsWith('/start')) {
        ...
        return new Response(JSON.stringify({
          redirectUri: 'https://broker.example.test/v1/oauth/linear/callback',
          expiresIn: 600,
        }), { status: 200 });
      }
      ...
      if (target === 'https://api.linear.app/oauth/token') {
        ...
        expect(body.get('redirect_uri')).toBe('https://broker.example.test/v1/oauth/linear/callback');
        expect(body.get('code_verifier')).toMatch(/^[A-Za-z0-9_-]{43}$/);
        return new Response(JSON.stringify({
    ...
    expect(authorizationUrl.searchParams.get('redirect_uri')).toBe('https://broker.example.test/v1/oauth/linear/callback');
