...
export async function startAuthorization({ origin } = {}) {
  ...
  if (!configuredRedirectUri) {
    throw new LinearOAuthError(
      'Linear OAuth callback is not configured. Set OPENCHAMBER_LINEAR_REDIRECT_URI or OPENCHAMBER_LINEAR_BROKER_URL for your own Linear app.',
      'LINEAR_REDIRECT_URI_MISSING',
    );
  }
...
export async function consumeAuthorizationCallback({ code, state, error, errorDescription }) {
  ...
  if (!pending?.codeVerifier) {
    throw failAuthorization(
      'This authorization session has expired or is unknown to the running app. Return to OpWrk and click Connect again.',
      'UNKNOWN_STATE',
    );
  }
