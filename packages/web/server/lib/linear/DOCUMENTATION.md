...
- Client ID: `OPENCHAMBER_LINEAR_CLIENT_ID` -> `settings.json` `linearClientId`. OpWrk has no inherited OAuth app default; connection fails with a configuration error until a client ID is supplied.
...
- Broker URL: `OPENCHAMBER_LINEAR_BROKER_URL` -> `settings.json` `linearBrokerUrl`. There is no hosted default. A custom broker is optional when a direct redirect URI is configured.
- Redirect URI: `OPENCHAMBER_LINEAR_REDIRECT_URI` -> `settings.json` `linearRedirectUri` -> `<broker-url>/callback` when a broker is configured. Without either setting, connection fails before any broker request. An explicit redirect URI bypasses the broker for custom/self-hosted OAuth applications.

Linear requires an exact callback match. Register the configured redirect URI with your own Linear OAuth application. With a custom broker, the broker holds only the short-lived authorization code; the local OpWrk server keeps the claim secret and PKCE verifier, exchanges the code for tokens locally, then acknowledges the handoff. Custom brokers must expose `/start`, `/callback`, `/poll`, and `/complete` with the same contract.
