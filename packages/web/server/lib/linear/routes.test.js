...
describe('Linear auth routes', () => {
  ...
  afterEach(() => {
    ...
    if (previousClientId === undefined) delete process.env.OPENCHAMBER_LINEAR_CLIENT_ID;
    else process.env.OPENCHAMBER_LINEAR_CLIENT_ID = previousClientId;
    fs.rmSync(dataDir, { recursive: true, force: true });
  });

  it('returns a configuration error before starting OAuth without a client id', async () => {
    delete process.env.OPENCHAMBER_LINEAR_CLIENT_ID;
    const app = createApp();
    const response = await request(app).post('/api/linear/auth/start').send({}).expect(400);
    expect(response.body.error).toContain('OPENCHAMBER_LINEAR_CLIENT_ID');
  });
