...
<title>${escapeHtml(title)} — OpWrk</title>
<style>
...
${desktopReturn ? `<a class="return" href="openchamber://focus/linear-auth">Return to OpWrk</a>
...
export function registerLinearRoutes(app) {
  ...
  app.get('/linear/oauth/callback', async (req, res) => {
    ...
    try {
      ...
      return finish(200, {
        ...
        message: 'You can close this tab and return to OpWrk.',
      ...
      return finish(status, {
        ...
        message: error instanceof Error ? error.message : 'Linear authorization failed. Return to OpWrk and click Connect again.',
  ...
  app.post('/api/linear/auth/start', parseJsonBody, async (req, res) => {
    try {
      ...
      const status = ['LINEAR_CLIENT_ID_MISSING', 'LINEAR_REDIRECT_URI_MISSING'].includes(error?.code) ? 400 : 500;
