...
`CsvPreview` renders a bounded, read-only table from the current draft in both
normal and fullscreen views. `parseCsvPreview` handles quoted and multiline
fields and reports when its character, row, column, or cell limits omit data.
Switching to edit mode exposes the untruncated draft; preview limits never
change the file read or save path.
