...
import { getWorkArtifactKind, type WorkArtifactKind } from './files/workArtifact';
import { CsvPreview } from './files/CsvPreview';
...
const WORK_ARTIFACT_LABEL_KEYS = {
  document: 'filesView.artifact.document',
  spreadsheet: 'filesView.artifact.spreadsheet',
  presentation: 'filesView.artifact.presentation',
} as const satisfies Record<WorkArtifactKind, I18nKey>;
...
export const FilesView: React.FC<FilesViewProps> = ({ mode = 'full', visible = true }) => {
  ...
  const handleOpenSelectedFile = React.useCallback(async () => {
    ...
    if (await openDesktopPath(selectedFile.path)) return;
    toast.error(t('filesView.artifact.openFailed'));
  }, [selectedFile?.path, t]);

  const handleDownloadSelectedFile = React.useCallback(async () => {
  ...
  }, [files.downloadFile, selectedFile, t]);
  ...
  const loadSelectedFile = React.useCallback(async (node: FileNode) => {
    ...
    await readFile(node.path)
      .then((content) => {
        ...
        const preferredTextMode = textViewModeByPathRef.current[node.path]
          ?? (node.path.toLowerCase().endsWith('.csv') ? 'view' : undefined);
        setTextViewMode(hasOwnPreviewMode ? 'edit' : initialFileTextMode(editorContent, preferredTextMode));
        setLoadedFilePath(node.path);
  ...
  const workArtifactKind = selectedFile?.path ? getWorkArtifactKind(selectedFile.path) : null;
  ...
  const isCsv = Boolean(selectedFile?.path && selectedFile.path.toLowerCase().endsWith('.csv'));
  ...
  const codePreviewActive = previewReady && canUseShikiFileView && !isCsv && textViewMode === 'view'
  ...
  const renderUnsupportedBinaryPreview = () => {
    const artifactLabel = workArtifactKind ? t(WORK_ARTIFACT_LABEL_KEYS[workArtifactKind]) : null;

    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center">
        {artifactLabel && selectedFile ? (
          <div className="mb-1 flex size-16 items-center justify-center rounded-2xl border border-border/50 bg-[var(--surface-subtle)]">
            <FileTypeIcon filePath={selectedFile.path} extension={selectedFile.extension} className="size-10" />
          </div>
        ) : null}
        <div className="typography-ui-header text-foreground">
          {artifactLabel
            ? t('filesView.artifact.readyTitle', { type: artifactLabel })
            : t('filesView.editor.cannotPreviewBinary')}
        </div>
        <div className="max-w-md typography-ui text-muted-foreground">
          {artifactLabel ? t('filesView.artifact.readyDescription') : t('filesView.editor.binaryFileDescription')}
        </div>
        <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
          {artifactLabel && !isBrowserClient ? (
            <Button type="button" size="sm" onClick={() => void handleOpenSelectedFile()}>
              <Icon name="external-link" className="mr-2 size-4" />
              {t('filesView.artifact.openDefaultApp')}
            </Button>
          ) : null}
          {files.downloadFile ? (
            <Button type="button" variant="outline" size="sm" onClick={() => void handleDownloadSelectedFile()}>
      ...
      <div className="flex-1 min-h-0 min-w-0 relative">
        <ScrollableOverlay ref={setMainPreviewScroller} outerClassName="h-full min-w-0" className={cn('h-full min-w-0', isLargeFile && '[overflow-anchor:none]')}>
          {!selectedFile ? (
            ...
            renderUnsupportedBinaryPreview()
          ...
          ) : selectedFile && isCsv && textViewMode === 'view' ? (
            <CsvPreview content={draftContent} fileName={selectedFile.name} />
