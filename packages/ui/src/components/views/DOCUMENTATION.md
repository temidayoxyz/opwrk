...
Office and OpenDocument files use the binary read path, but they are presented
as finished deliverables rather than generic binary failures. Documents,
spreadsheets, and presentations show a type-aware card. Browser clients keep
the save/download action; Electron also offers a direct open in the operating
system's default app through the existing desktop bridge. Unknown binary files
retain the generic fallback.

CSV files open in a read-only table preview by default on every shared UI
runtime. The existing preview/edit toggle exposes the complete text draft for
editing. `files/parseCsvPreview.ts` bounds parsed characters, rows, columns,
and cell length; it never truncates the editable draft or saved file. Both the
normal and fullscreen file views use the same preview component.
