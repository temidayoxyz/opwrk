...
export const formatAgentDisplayName = (agentName: string, labels: AgentDisplayLabels) => {
    if (agentName === 'build') return labels.work;
    if (agentName === 'plan') return labels.plan;
    return capitalizeLabel(agentName);
};

export const getAgentDisplayName = (agents: Agent[], labels: AgentDisplayLabels, agentName?: string) => {
    if (agentName) {
        const agent = agents.find((entry) => entry.name === agentName);
        return formatAgentDisplayName(agent?.name ?? agentName, labels);
    }

    const primaryAgents = agents.filter((agent) => isPrimaryMode(agent.mode));
    const buildAgent = primaryAgents.find((agent) => agent.name === 'build');
    const fallbackAgent = buildAgent || primaryAgents[0] || agents[0];
    return fallbackAgent ? formatAgentDisplayName(fallbackAgent.name, labels) : labels.selectAgent;
};
