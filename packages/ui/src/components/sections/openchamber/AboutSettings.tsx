...
const GITHUB_URL = 'https://github.com/temidayoxyz/opwrk';
...
export const AboutSettings: React.FC<AboutSettingsProps> = ({ initialUpdateDialogOpen = false }) => {
  ...
  const [opWrkVersion, setOpWrkVersion] = React.useState<string | null>(null);
  ...
  const currentVersion = opWrkVersion || updateStore.info?.currentVersion || 'unknown';

  React.useEffect(() => {
    let cancelled = false;

    const loadOpWrkVersion = async () => {
      try {
        ...
        const version = typeof data?.openchamberVersion === 'string' && data.openchamberVersion.trim().length > 0
          ...
          : null;
        if (!cancelled) setOpWrkVersion(version);
      } catch {
        if (!cancelled) setOpWrkVersion(null);
      }
    };

    void loadOpWrkVersion();
    ...
    return (
      <div className="w-full space-y-6 pb-2">
        <div className="flex flex-col items-center text-center">
          ...
          <h2 className={`mt-4 ${SETTINGS_BRAND_TITLE_CLASS}`}>OpWrk</h2>
