...
import { formatAgentDisplayName } from '@/components/chat/mobileControlsUtils';
...
export const AgentSelector: React.FC<AgentSelectorProps> = ({
    ...
    const agentDisplayLabels = {
        work: t('chat.modelControls.agent.work'),
        plan: t('chat.modelControls.agent.plan'),
        selectAgent: t('chat.modelControls.selectAgent'),
    };
    const formatAgentName = (name: string) => formatAgentDisplayName(name, agentDisplayLabels);
                        ...
                        return (
                            ...
                            >
                                <div className="flex flex-col">
                                    <span className="typography-meta font-medium">{formatAgentName(agent.name)}</span>
                                    {agent.description && (
            ...
            {isActuallyMobile ? (
                        ...
                        {!isReady && !agentName ? (
                            ...
                            <>
                                ...
                                <span className="typography-meta font-medium text-foreground">
                                    {agentName ? formatAgentName(agentName) : t('settings.commands.agentSelector.selectAgentPlaceholder')}
                                </span>
                            </>
                        )}
                    </div>
                ...
                <div className={cn(
                    ...
                    <span className="typography-micro font-medium whitespace-nowrap text-muted-foreground">
                        {agentName ? formatAgentName(agentName) : (isUnavailable ? t('common.unavailable') : t('common.loading'))}
                    </span>
                </div>
            ) : (
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <div className={cn(
                            ...
                            <span className="typography-micro min-w-0 flex-1 truncate text-left font-medium">
                                {agentName ? formatAgentName(agentName) : t('settings.commands.agentSelector.notSelected')}
                            </span>
