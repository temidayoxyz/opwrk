...
import {
  CONTEXT_SURFACES,
  DEFAULT_CONTEXT_RAIL_HIDDEN_SURFACES,
...
describe('getVisibleContextRailSurfaces', () => {
  ...
  test('puts work surfaces before developer surfaces in the default order', () => {
    const ids = getVisibleContextRailSurfaces(baseOptions).map((surface) => surface.id);
    expect(ids.slice(0, 6)).toEqual(['editor', 'browser', 'notes', 'context', 'plan', 'linear']);
    expect(ids.indexOf('git')).toBeGreaterThan(ids.indexOf('linear'));
  });

  test('hides developer panels in the fresh OpWrk experience', () => {
    const ids = getVisibleContextRailSurfaces({
      ...baseOptions,
      hiddenSurfaces: DEFAULT_CONTEXT_RAIL_HIDDEN_SURFACES,
    }).map((surface) => surface.id);

    expect(ids).toEqual(['editor', 'browser', 'notes', 'context', 'plan', 'linear']);
  });
