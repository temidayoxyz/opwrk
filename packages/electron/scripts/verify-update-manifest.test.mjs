...
for (const [manifestName, artifactName] of [
  ['latest-linux.yml', 'OpWrk-1.15.0-linux-x86_64.AppImage'],
  ['latest-linux-arm64.yml', 'OpWrk-1.15.0-linux-arm64.AppImage'],
]) {
...
test('accepts electron-builder field ordering and optional blockMapSize', () => {
  const artifactName = 'OpWrk-1.15.0-linux-x86_64.AppImage';
  const bytes = Buffer.from(`artifact:${artifactName}`);
...
test('rejects a manifest that points at the other architecture artifact', () => {
  const value = fixture('latest-linux-arm64.yml', 'OpWrk-1.15.0-linux-arm64.AppImage');
  try {
    const x64Artifact = path.join(value.root, 'OpWrk-1.15.0-linux-x86_64.AppImage');
