...
const createPayload = () => {
  ...
  fs.writeFileSync(path.join(root, 'opwrk.desktop'), [
    '[Desktop Entry]', 'Name=OpWrk', 'Exec=AppRun --no-sandbox %U', 'Icon=opwrk', 'StartupWMClass=opwrk', '',
  ].join('\n'));
  writeElf(path.join(root, 'opwrk'), 'x64');
