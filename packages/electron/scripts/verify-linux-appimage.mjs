...
export const verifyExtractedPayload = ({
  ...
  const desktopPath = path.join(root, 'opwrk.desktop');
  ...
  for (const entry of ['Name=OpWrk', 'Icon=opwrk', 'StartupWMClass=opwrk']) {
  ...
  assertElfArchitecture(path.join(root, 'opwrk'), targetArchitecture, 'Electron executable');
...
const findAppImage = (version, architecture) => {
  ...
  const expected = path.join(electronRoot, 'dist', `OpWrk-${version}-linux-${suffix}.AppImage`);
