...
describe('ElectronSshManager', () => {
  ...
  test('never installs the inherited package on a remote host', async () => {
    const commands = [];
    const manager = new ElectronSshManager({
      settingsFilePath: path.join(os.tmpdir(), 'unused-settings.json'),
      appVersion: '1.2.3',
    ...
    await expect(manager.installOpenChamberManaged()).rejects.toThrow(/Managed remote setup is unavailable/);
  ...
  test('refuses managed mode before probing or starting a remote OpenChamber binary', async () => {
    const commands = [];
    const manager = new ElectronSshManager({
      settingsFilePath: path.join(os.tmpdir(), 'unused-settings.json'),
      appVersion: '1.2.3',
    ...
    await expect(manager.ensureRemoteServer(
      { id: 'ssh-1', remoteOpenchamber: { mode: 'managed' } },
      { destination: 'user@example.test', args: [] },
      '/tmp/control.sock',
    )).rejects.toThrow(/Connect to an existing OpWrk server in External mode/);
