...
const PACKAGED_APP_USER_MODEL_ID = 'dev.opwrk.desktop';
const DEV_APP_USER_MODEL_ID = 'dev.opwrk.desktop.dev';
...
// ~/Library/Logs/OpWrk/ (not ~/Library/Logs/@openchamber/electron/).
app.setName('OpWrk');
if (process.platform === 'linux') {
  app.setDesktopName('opwrk.desktop');
}
if (isDev) {
  app.setPath('userData', path.join(app.getPath('appData'), 'OpWrk Dev'));
}
...
// that output never lands in ~/Library/Logs/OpWrk/main.log and we
...
const GITHUB_BUG_REPORT_URL = 'https://github.com/temidayoxyz/opwrk/issues/new?template=bug_report.yml';
const GITHUB_FEATURE_REQUEST_URL = 'https://github.com/temidayoxyz/opwrk/issues/new?template=feature_request.yml';
...
const quitConfirmationMessage = () => {
  ...
  return `OpWrk detected ${reasons.join(', ')}. Quitting now will stop sidecar/background processes and may interrupt pending work.`;
};
...
// session expires) reuses the same "OpWrk Desktop" record instead of
...
/**
 * The browser panel's own session, kept separate from OpWrk's.
 *
...
const buildStartupSplashHtml = () => {
  ...
  <body>
    <div class="stack">
      <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="OpWrk loading icon">
...
const parseConnectPairingDeepLinkPayload = (raw) => {
  ...
  try {
    ...
    return {
      pairingId,
      secret,
      label: typeof payload.label === 'string' && payload.label.trim() ? payload.label.trim() : 'OpWrk',
...
const handleInvoke = async (browserWindow, command, args = {}) => {
  switch (command) {
    ...
    // on the remote OpWrk host, so the browser panel loads a real origin
     ...
     *
     * nativeTheme.themeSource is app-wide and would drag OpWrk's own
     ...
     * to third-party hosts from OpWrk itself. The bytes come back as a
    ...
    // touch OpWrk's session or any other window's storage.
      ...
      if (applyUpdate && process.platform === 'darwin' && typeof app.isInApplicationsFolder === 'function') {
        try {
          if (!app.isInApplicationsFolder()) {
            throw new Error('Desktop update requires OpWrk.app to be installed in /Applications');
          }
        } catch (error) {
  ...
  return Menu.buildFromTemplate([
    {
      label: app.name,
      submenu: [
        { label: 'About OpWrk', click: () => dispatchAction('about') },
        {
