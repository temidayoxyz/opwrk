...
const waitLocalForwardReady = async (localPort) => {
  ...
  throw new Error('Timed out waiting for forwarded OpWrk health');
};
...
export class ElectronSshManager {
  ...
  sanitizeInstance(instance) {
    ...
    return {
      ...
      remoteOpenchamber: {
        mode: instance?.remoteOpenchamber?.mode === 'managed' ? 'managed' : 'external',
  ...
  async issueClientToken(localUrl, openchamberPassword) {
    ...
    const loginResponse = await fetch(new URL('/auth/session', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({
        ...
        clientLabel: 'OpWrk Desktop SSH',
      }),
    });
    if (!loginResponse.ok) {
      throw new Error(`Configured OpWrk UI password was rejected by forwarded server (status ${loginResponse.status})`);
    }
    ...
    const tokenResponse = await fetch(new URL('/api/client-auth/clients', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({ label: 'OpWrk Desktop SSH' }),
    });
  ...
  // Every place the inherited OpenChamber runtime may live on the remote host,
  // with the version each
  ...
  async installOpenChamberManaged() {
    throw new Error('Managed remote setup is unavailable until OpWrk publishes its own server package. Connect to an existing OpWrk server in External mode.');
  }
    ...
    if (isLivenessHttpStatus(infoStatus)) {
      if (isAuthHttpStatus(infoStatus)) {
        if (openchamberPassword && authStatus !== 200) {
          throw new Error(`Remote OpWrk requires UI authentication and configured password was rejected (auth status ${authStatus})`);
        }
        if (isLivenessHttpStatus(healthStatus)) return {};
        throw new Error('Remote OpWrk requires UI authentication on /api/system/info; configure the OpWrk UI password');
      }
    } else if (isLivenessHttpStatus(healthStatus)) {
      return {};
    } else {
      throw new Error(`Remote OpWrk probe failed (info status ${infoStatus}, health status ${healthStatus})`);
    }
  ...
  async ensureRemoteServer(instance, parsed, controlPath) {
    ...
    if (!instance.remoteOpenchamber.preferredPort) {
      throw new Error('External mode requires a preferred remote OpWrk port');
    }
    const port = instance.remoteOpenchamber.preferredPort;
    this.setStatus(instance.id, 'server_detecting', 'Probing external OpWrk server', null, null, port, false, 0, false);
