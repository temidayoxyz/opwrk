...
//
// Surfaces a glanceable, always-visible view of OpWrk's live state:
...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'OpWrk — no active sessions';
  const bits = [];
  ...
  return `OpWrk — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    if (isLinux || process.platform === 'win32') {
      ...
      template.push(
        { label: 'Show OpWrk', click: () => onAction({ type: 'show-main-window' }) },
        { type: 'separator' },
        { label: 'Quit OpWrk', click: () => onAction({ type: 'quit' }) },
      );
    }
