...
Electron desktop runtime for OpWrk on macOS, Windows, and Linux.

This package owns OpWrk's native shell: windows, menus, deep links, native notifications, auto-updates, host switching, SSH connections, tunnel helpers, and packaged desktop builds. The web server logic still lives in `packages/web`, and the React UI shared with browser/PWA builds lives in `packages/ui`.

## How It Runs

Desktop starts the inherited web server in the same Electron main process. There is no separate sidecar subprocess for the application server.
...
Managed SSH setup is paused until OpWrk publishes its own server package. External mode can still connect to a server the user already runs. The desktop must never install the inherited `@openchamber/web` package on a remote host.
...
Linux AppImages must be built natively. Set `OPENCHAMBER_TARGET_ARCH=x64` or `OPENCHAMBER_TARGET_ARCH=arm64` when packaging; the build rejects a target that does not match the Linux host. The same target selects the bundled OpenCode CLI, native Electron rebuild, and Electron Builder architecture. Linux identity is stable across architectures: executable `opwrk`, desktop file `opwrk.desktop`, icon `opwrk`, and `StartupWMClass=opwrk`.
...
Electron uses `electron-log`. In development, console logs are also visible in the terminal. In packaged apps, logs are written through the platform log path for the `OpWrk` app name.

Development builds use a separate user data directory named `OpWrk Dev`, so dev state does not overwrite normal packaged app state.

The inherited `OPENCHAMBER_*` environment variables, preload bridge, custom UI protocol, and some storage keys are compatibility namespaces for now. They are internal implementation details, not the installed product identity; rename them only through a focused migration that preserves existing local data and runtime interoperability.
