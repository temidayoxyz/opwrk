...
| `surface` | 在侧栏中为 `panel`，在附加窗口中为 `dialog` |
...
`startSession` 在当前项目中创建会话。设置 `worktree: true` 时，OpenChamber 会先创建一个 git worktree，并在那里打开会话。设置了 `text` 且已选择模型时，它会作为第一条消息发送。结果是 `{ sessionId, sent }`，其中 `sent` 为 `sent`、`no-model`、`skipped` 或 `failed`。即使结果是 `no-model` 或 `failed`，会话也已存在。没有打开的项目、worktree 创建失败或会话创建失败，都是 `HOST_REJECTED` 错误。
