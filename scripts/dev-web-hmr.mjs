...
const stagedOpenCodeBinary = path.join(
  repoRoot,
  'packages/electron/resources/opencode-cli',
  process.platform === 'win32' ? 'opencode.exe' : 'opencode',
);
const useStagedOpenCode = !process.env.OPENCODE_BINARY?.trim() && existsSync(stagedOpenCodeBinary);
const isolatedDevDataDir = path.join(os.tmpdir(), 'opwrk-dev-data');
...
if (useStagedOpenCode) {
  apiEnvironment.OPENCODE_BINARY = stagedOpenCodeBinary;
  apiEnvironment.XDG_DATA_HOME = process.env.XDG_DATA_HOME || isolatedDevDataDir;
  apiEnvironment.XDG_CONFIG_HOME = process.env.XDG_CONFIG_HOME || isolatedDevDataDir;
  apiEnvironment.OPENCHAMBER_DATA_DIR = process.env.OPENCHAMBER_DATA_DIR || path.join(isolatedDevDataDir, 'settings');
}
...
if (useStagedOpenCode) {
  console.log('[dev:web:hmr] Using the staged OpenCode runtime with isolated development data');
}
