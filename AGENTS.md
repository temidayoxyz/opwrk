...
OpWrk is a local-first work agent powered by OpenCode. It directly forks OpenChamber and currently retains its shared web, desktop, VS Code, hosted-mobile, and native-mobile package structure.
...
During the fork migration, internal `openchamber` package names, environment
variables, and runtime identifiers remain authoritative until a focused change
replaces them. Product copy and new public contracts use OpWrk. Do not perform
an untested repository-wide rename.

## OpWrk product invariants

- Core local use must work without an OpWrk account or hosted control plane.
- Provider credentials and model configuration remain local unless the user
  explicitly chooses a remote provider or service.
- The user-facing agent modes are Work and Plan. Work replaces Build in product
  copy; Plan remains the read-only mode.
- Documents, PDFs, spreadsheets, presentations, structured data, images, and
  cited research reports are first-class artifacts.
- Shared UI changes are complete only after web and Electron behavior are both
  checked. Native-only changes require Electron-specific validation.
- Developer tools may remain available behind progressive disclosure, but the
  default experience must not require Git or shell knowledge.
